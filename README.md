...
$$\sum_{i=m}^n \left(\frac{1}{\left(\sum_{j\leq i}\lambda_j\right)^2}\right)\left[\left(\left(\sum_{j\leq i}\lambda_j\right)x-1\right)\exp\left(N + \sum_{j\leq i} \lambda_j(x-k_j)\right)\right]_{k_i}^{k_{i+1}}= C_m$$
